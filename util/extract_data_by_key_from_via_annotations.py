...
def extract_data_by_key_from_via_annotations(annotations_path, key, output_path):
    ...
    with open(annotations_path, "r") as json_file, open(os.paht.join(output_path, modified_annotations_file_name), "w") as json_output_file:
        json_data = json.load(json_file)
        output_json_data = {entry : json_data[entry] for entry in json_data if key in entry}
        print("Copied the following entries:")
        for key in json_data: print(key)
