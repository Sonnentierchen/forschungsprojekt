    ...
    with open(annotationsPath) as annotationData:
        ...
        for entry in data:
            ...
            for regionIndex in regions:
                ...
                box_x = int(shapeAttributes["x"])
                box_y = int(shapeAttributes["y"])
                # VIA data can be negative numbers or larger than image size
                box_x = max(box_x, 0)
                box_y = max(box_y, 0)
                box_width = int(shapeAttributes["width"])
                box_height = int(shapeAttributes["height"])
                #print("{} {} {} {}".format(box_x, box_y, box_width, box_height))
                # Adjust to regions that go beyond the image
                if box_x + box_width > width:
                  box_width = width - box_x
                if box_y + box_height > height:
                  box_height = height - box_y
                if not box_width < 3 and not box_height < 3:
                  # filter out boxes that are too small
                  cocoData["annotations"].append({"segmentation" : [[box_x, box_y, \
                                                                    box_x + box_width, box_y, \
                                                                    box_x + box_width, box_y + box_width, \
