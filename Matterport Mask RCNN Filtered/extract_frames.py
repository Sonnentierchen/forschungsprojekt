...
def storeImage(image, frameId, numberOfFrames, frames, outputImagesPath):
	frames.append({'image' : image,
			'frameNumber' : frameId})
	if outputImagesPath:
		# Add leading zeros, as many as needed by the total amount of frames
		numberOfFramesString = str(numberOfFrames)
		cv2.imwrite(outputImagesPath + "_frame_" + str(frameId).zfill(len(numberOfFramesString)) + ".jpg", image)
...
def extract_frames(videoPath, outputImagesPath, maxNumberOfFrames=0, distribute="even"):
	...
	maxNumberOfFrames = length if maxNumberOfFrames == 0 else maxNumberOfFrames
	...
	while success:
		...
		if distribute == "even" and frameId % skipFrames == 0:
			...
			storeImage(image, frameId, length, frames, outputImagesPath)
		# Somehow the last frame is always broken...
